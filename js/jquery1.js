	...
	$(".comment-btn").each(function(){
		$(this).click(function(e){
			...
			function countComments(newsId) {
				$.ajax({
					...
					success: function(data) {
						...
						document.getElementById("num_com").innerHTML = data;
					}
				});
			...
			function comment_insert(data) {
				...
				var z = '';
				var comment_img = '#comment-img' + newsId;
				z += '<div id="' + data.comment.comment_id + '" class="comment-img1">';
				z += '<div class="profile"><img src="images/head.png"></div>';
				z += '<div class="display"><p class="name">' + data.user.username;
				z += ': </p><p class="name-p">' + data.comment.comment; + ' - (';
				z += ' (' + day + '.' + ("0" + month).slice(-2);
				z += '.' + year + ', ' + hour + ':' + min + ')';
				z += '<p id="';
				z += data.comment.comment_id;
				z += '" class="delete-comment">X</p>';
				z += '</p></div></div>';

				$(comments_see).prepend(t);
				$(comment_img).prepend(z);
				add_delete_handlers();
				countComments(newsId);
